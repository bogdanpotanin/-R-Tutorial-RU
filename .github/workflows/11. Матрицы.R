...
# Структуры данных в R это особые объекты. 
# Они состоят из других объектов и обычно отличаются
# между собой типом хранимых объектов и способами
# доступа к ним.

# Матрицы как и вектор содержат объекты одного
# и того же типа, но отличаются от векторов тем, 
# что имеют два измерения (two-dimensional)
...
# Матрица также имеет тип double и состоит из
# векторов типа double, содержащих один элемент
typeof(m_1)
is.double(m_1[3, 2])
class(m_1)
...
z <- c(1, 2, 3)
class(z)                        # было numeric
z <- as.matrix(z)               # преобразуем класс
                                # объекта z в matrix
class(z)                        # стало matrix
...
# использовании конструкции drop = FALSE
# позволяет предотвратить превращение
# столбцов и строк матрицы в векторы
m_1[1, , drop = FALSE]              # возвращаем матрицу 1xN
dim(m_1[1, , drop = FALSE])
m_1[, 2, drop = FALSE]              # возвращаем матрицу Nx1
dim(m_1[, 2, drop = FALSE])
m_1[1, , drop = FALSE] %*% m_1      # работает правильно
matrix(m_1[1, ], nrow = 1) %*% m_1  # работает правильно, поскольку в
...
# Строкам и столбцам матрицы можно задать имена
# при помощи изменения атрибута dimnames()
# используя функции colnames() и rownames()
rownames(m_1) <- c("rhino", "fish", "rabbit")
colnames(m_1) <- c("cat", "dog", "bird")
print(m_1)
m_1[c("rhino", "fish"), c("bird", "cat")]  # вы можете обращаться к столбцам
                                           # и строкам матрицы по именам

# Чтобы присвоить элементам матрицы другие
# значения достаточно использовать оператор
# присваивания <-
m_1[2, 3] <- 8                                     # работает
m_1[c(1, 3), c(3, 2)] <- m_2[-1, c(1, 3)]          # работает
m_1[c(1, 3), c(3, 2)] <- m_1[c(1, 3), c(3, 2)] +   # работает
                         m_2[-1, c(1, 3)]
m_1[c(1, 3), c(3, 2)] <- m_1[c(1, 3), ]            # не работает, поскольку
dim(m_1[c(1, 3), c(3, 2)])                         # не совпадают размерности
dim(m_1[c(1, 3), ])

# При помощи функции array() можно создавать массивы
a_1 <- array(c(1, 2, 3,            # алгоритм превращения вектора в
               4, 5, 6,            # трехмерный массив предполагает, что
               10, 11, 12,         # по порядку заполняются третьи измерения
               13, 14, 15,         # матрицами по столбцам (byrow = FALSE)
               100, 101, 
               102, 103),
             dim = c(2, 3, 3))
a_1[2, 3, 2]                      # для получения доступа к элементу
...
eigen(m_1)         # лист, содержащий собственные
                   # числа матрицы (values) и
                   # собственные векторы (vectors)
kappa(m_1)         # число обусловленности (conditional number)
diag(m_1)          # диагональ матрицы
    ...
    #    функция должна возвращать ошибку с помощью функции stop().
{
  ...
  myFunc(m_3[, -1])              # не работает, так как не квадратная
    ...
    #    а возвращаемым объектом - матрица их произведения (как в линейной алгебре), 
    ...
    #    квадратной матрицей.
{
  ...
  myFunc(mat_1, mat_2[,c(1, 2)])    # матрицы mat_1 и mat_2[c(1, 2),]
    ...
    # 9. При помощи функции eigen() напишите функцию, возвращающую
    #    наибольшее из собственных чисел матрицы
{
  myFunc <- function (m)
  {
    eigen_list <- eigen(m)
    max_val <- max(eigen_list$values)
    return(max_val)
  }
  myFunc(matrix(c(1, -1, 3, 8), ncol = 2))
}