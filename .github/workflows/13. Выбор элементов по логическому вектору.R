...
# к элементам вектора
v_1 <- c(5.1, 4.3, 2.1, 1.1, 3.5)             # создаем вектор
b_1 <- c(TRUE, FALSE, TRUE, FALSE, FALSE)     # создаем логический вектор
v_1[b_1]                                      # выбираем значения v_1
                                              # для которых b_1 равен TRUE
v_2 <- c(6.1, 9.2, 8, 7.2, 10)                # другой вектор
(v_2 > 7.5)                                   # логическое выражение 
                                              # вернуло логический вектор
...
d <- data.frame(Some_Column = c(1, 7, 1, -5 ,6), 
                Other_Column = c(10, 3, 5, 15, 32))
# Выберем только те строки, где все значения больше 2
d[(d[, 1] > 2) & (d[, 2] > 2), ]
...
cond_1 <- which(v_1 > 6.5)              # индексы v_1, которым соответствуют
                                        # элементы, значения которых
                                        # превышают 6.5
v_1[cond_1]
cond_2 <- which(d$Some_Column > 1)      # индексы столбца Some_Column, 
